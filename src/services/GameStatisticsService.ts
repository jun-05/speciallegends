...
class GameStatisticsService {
  ...
  analyzeData(data: (string | number)[][]): SmasherData {
    ...
    for (const row of data) {
      ...
      if (!results[tier]) {
        results[tier] = { maps: {}, characters:{},  totalGamesInTier: 0,totalGameWin:0 };
      }
      ...
      if (!results[tier].maps[mapNumber].characters[charNumber]) {
        results[tier].maps[mapNumber].characters[charNumber] = { gameCount: 0, winCount: 0 };
      }
    ...
    for (const tier of Object.values(smasherData)) {
      for (const mapData of Object.values(tier.maps)) {
        for (const characterResult of Object.values(mapData.characters)) {
          characterResult.winRate = Math.round((characterResult.winCount / characterResult.gameCount) * 1000) / 10;
          characterResult.pickRate =  Math.round((characterResult.gameCount / (mapData.totalGamesInMap/6) ) * 1000) / 10;
        }
      }

      for (const characterResult of Object.values(tier.characters)) {
        characterResult.winRate = Math.round((characterResult.winCount / characterResult.gameCount) * 1000) / 10;
        characterResult.pickRate =  Math.round((characterResult.gameCount / (tier.totalGamesInTier/6)) * 1000) / 10;
      }
    }
  }
}
