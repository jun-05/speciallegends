...
const Tooltip = ({ title, infoText, idx, position }: TooltipProps) => {
  return (
    <div
      className={`flex flex-col absolute mb-2 -ml-1 md:ml-0 text-start tooltip  max-w-[256px] w-auto  px-2 py-1 md:p-4 bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-white rounded-lg shadow-lg z-50 bottom-full left-1/2 transform md:-translate-x-1/2  ${
        position === 'left'
          ? (idx! + 1) % 3 === 0
            ? ' -translate-x-1/2'
